class Autoship < ApplicationRecord
  ...
  validates :payment_card_id, presence: { message: 2013005 }, if: -> { self.payment_method == "card" }
  validates :recurring_interval_count, presence: { message: 2013006 }, numericality: { in: 5..90, message: 2013007 }, if: -> { self.recurring_interval == "day" }
  validates :recurring_interval_count, presence: { message: 2013006 }, numericality: { in: 1..3, message: 2013007 }, if: -> { self.recurring_interval == "month" }
end