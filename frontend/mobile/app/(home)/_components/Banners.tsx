...
const Banners = ({ data }: BannersProps) => {
  ...
  const renderRightBanner = useCallback(() => {
    return (
      <Image
        style={{
          flex: 1,
  ...
  return (
    <View className="bg-black mx-[28px] rounded-[4px]">
      <Swipeable
        childrenContainerStyle={{
          height: 136,
          width: "100%",
        }}
        onSwipeableOpen={(d) => console.log("open", d)}
        onSwipeableClose={(d) => console.log("close", d)}
        onSwipeableWillClose={(d) => console.log("will", d)}
        renderRightActions={renderRightBanner}
      >
        {renderBanner()}
      </Swipeable>
    </View>
